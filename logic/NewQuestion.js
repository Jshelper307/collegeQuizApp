...
document.addEventListener("DOMContentLoaded",()=>{
    ...
    subjectName.innerHTML = localStorage.getItem("subjectName");
})
...
function loadQuestion(unit, topic, questionIndex) {
    if(isTeacher){
        showAllWQueston(topic);
    }
    else{
...
function calculateScore() {
    score = responses.reduce(
        (total, response, idx) =>
            total +
            (response === currentTopicQuestions[idx].correct ? 1 : 0),
        0
    );
}
    ...
    optionsContainer.innerHTML = responses
        .map((response, idx) => {
            ...
            return `<p style="color: ${
                isCorrect ? "green" : "red"
            };">Q${idx + 1}: ${currentTopicQuestions[idx].question}<br> Your Answer: ${
                response !== null
        ...
        .join("");
    feedbackContainer.textContent = `Time left: ${minutes}:${seconds < 10 ? '0' + seconds : seconds}`;
...
addUnitBtn.addEventListener("click",()=>{
    addUnitDiv.style.display = addUnitDiv.style.display == "block"?"none":"block";
})
...
addTopicBtn.addEventListener("click",()=>{
    addTopicDiv.style.display = addTopicDiv.style.display == "block"?"none":"block";
})
...
addNewQuestionBtn.addEventListener("click",()=>{
    ...
    console.log("Options : ",optionOne," ",optionTwo," ",optionThree," ",optionFour);
    ...
    if(addedQuestions.length==0){
        ...
        addedQuestions.map(question=>{
            return showAddedQuestionsDiv.innerHTML += `<div class="question">
                        ...
                        <div class="showOptions">
                            <p class=${question['correct']==0 && 'green'}>1. ${question['options'][0]}</p>
                            <p class=${question['correct']==1 && 'green'}>2. ${question['options'][1]}</p>
                        </div>
                        <div class="showOptions">
                            <p class=${question['correct']==2 && 'green'}>3. ${question['options'][2]}</p>
                            <p class=${question['correct']==3 && 'green'}>4. ${question['options'][3]}</p>
                        </div>
                    </div>`
        })
    }
